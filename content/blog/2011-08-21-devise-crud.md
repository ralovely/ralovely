...
I found out that it needs a little correction and an addendum.
* The path_prefix in the routes file is not optional. If you don't add it, rails will mix up edit routes.
* In your controller's update method, you need to check if the password is blank. If so, remove the key from the hash. If not, Devise will fail its validations.
