...
There are three ways to give (a bit of) control to the user.  
...
The second is to prompt the user while executing the script. This is powerful, sometimes mandatory, but often a smell for bad design at the beginning. Use it carefully, mainly to prompt for information that depends on the execution itself (ie. `File already exists, what should I do`). Avoid using it on long running scripts, where the user will do something else meanwhile, missing the prompt.
...
It is a time based control, and it's usually not wise to race with a machine doing a couple of tera calculations per second. Yet, it can sometimes save the day.  
...
For starters, it's an excellent form of documentation. I learned a ton about Capistrano, just reading the output while it was running.  
...
## Logging & Monitoring
Retroactive analysis can be a life saver, or at minimum a good tool to improve the system.  
...
This can be a good indication of a rising problem. If your script suddenly takes twice the time as usual, find why.  
...
Test it against an environment too, virtual machines are cheap and easy.  
...
At this stage I still trigger the whole script manually, usually for a fortnight.  
